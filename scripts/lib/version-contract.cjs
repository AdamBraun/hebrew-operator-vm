...
const VERSION_SOURCE_PATH = path.resolve(process.cwd(), "src", "reference", "version.ts");
...
function parseVersionContractSource(sourceText) {
  ...
  if (!trace_version || !semantics_version || !render_version) {
    throw new Error(
      "Unable to parse TRACE_VERSION/SEMANTICS_VERSION/RENDER_VERSION from src/reference/version.ts"
    );
  }
