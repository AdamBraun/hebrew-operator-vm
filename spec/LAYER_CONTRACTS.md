...
| ------------------------------ | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Spine (Normalization)          | `src/spine/**`, `src/cli/build-spine.ts`                                              | Raw corpus text + normalization options                                  | canonical latest `spine.jsonl` (`outputs/runs/latest/spine.jsonl`) plus internal cache metadata | No semantic selection (no layout/cantillation/VM semantics) |
| Letters                        | `src/layers/letters/**`, `src/cli/build-layer.ts --layer letters`                     | Spine grapheme/gap stream                                                | canonical latest `letters.ir.jsonl` plus internal cache metadata                                | No niqqud/cantillation/layout semantics                     |
| Niqqud                         | `src/layers/niqqud/**`, `src/cli/build-layer-niqqud.ts`                               | Spine grapheme stream (`marks_raw.niqqud`)                               | canonical latest `niqqud.ir.jsonl` plus optional quality views in cache                         | No gap semantics and no peer-layer joins                    |
| Cantillation                   | `src/layers/cantillation/**`, `src/cli/build-layer.ts --layer cantillation`           | Spine grapheme+gap stream (`marks_raw.teamim`, gap punctuation evidence) | canonical latest `cantillation.ir.jsonl` plus internal cache metadata                           | No layout selection and no runtime semantics                |
| Layout                         | `src/layers/layout/**`, `src/cli/build-layer.ts --layer layout`                       | Spine gaps + layout dataset                                              | canonical latest `layout.ir.jsonl` plus internal cache metadata                                 | No trope/niqqud/VM interpretation                           |
| Metadata                       | `src/layers/metadata/**`, `src/cli/build-layer-metadata.ts`                           | Plan dataset + canonical Torah ref order                                 | canonical latest `metadata.plan.jsonl` plus internal cache metadata                             | No anchor-level runtime semantics                           |
| Stitcher / Wrapper composition | `src/wrapper/stitch/**`, `src/wrapper/program_schema.ts`, `src/cli/stitch-program.ts` | Spine + all orthogonal IRs + MetadataPlan                                | transient `ProgramIR.jsonl` view build; no repo-tracked stitched artifact contract              | No new semantic inference beyond deterministic join rules   |
